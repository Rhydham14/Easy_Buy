        ...
        <Row>
          ...
          {cat.map((category, index) => (
            <Col key={index} className="text-center mb-4">
              <h2 style={{backgroundColor:"white", color:"purple"}}>{category}</h2>
              <Row className="justify-content-center" style={{ overflowX: "auto" }}>
                {filterProductsByCategory(category).slice(0, 10).map((product, productIndex) => (
                  <Col key={productIndex} xs={12} sm={6} md={4} lg={3} className="mb-4 ">
                    <Link to={`/details/${product._id}`} style={{ textDecoration: "none" }}>
                      <Card className="h-100 p-4" id="card-image">  
                        ...
                        <Card.Body>
                          ...
                          <Card.Text className="text-truncate" style={{ height: "50px", overflow: "hidden" }}>
                            {product.description}
                          </Card.Text>
                          <Card.Text>₹ {product.price}/-</Card.Text>
                          <Button as="div" variant="primary" style={{ backgroundColor: "purple", border: "none" }}>
                            More
                          </Button>
                        </Card.Body>
                      </Card>
